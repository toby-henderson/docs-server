...
(See xref:security-authorization.adoc[Authorization] for more information on
RBAC.)
...
. Download link:{attachmentsdir}/x509-cert-scripts/gen_certificates.sh[gen_certificates.sh], link:{attachmentsdir}/x509-cert-scripts/openssl-san.cnf[openssl-san.cnf] and link:{attachmentsdir}/x509-cert-scripts/v3_ca.ext[v3_ca.ext] to the *SSLCA* directory.
